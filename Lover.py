...
for xd in range(10000):
	...
	c=f' en-us; {str(gt)}'
	g='AppleWebKit/537.36 (KHTML, like Gecko) Samsung'/'
...
def pak():
		...
		for nmbr in range(limit):
			...
			user.append(nmp)
		with tred(max_workers=3) as TRT:	
			clear()
			tl = str(len(user))
...
def api1(ids,names,passlist):
		try:
			global ok,loop
			sys.stdout.write('\r\r\033[1;35m [Lover-M1] %s|\033[1;35mOK:-%s \033[1;35m'%(loop,len(oks)));sys.stdout.flush()
			fn = names.split(' ')[0]
			try:
				ln = names.split(' ')[1]
			except:
